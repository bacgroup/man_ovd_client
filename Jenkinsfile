...
EOF"""
    ...
    stage "Archive Artifacts"
     dir ('packages') {
        sh "mv *darwin*.zip \"MAN_ADS_Client_2.0-OSX_Java_Client.zip\""
        sh "mv *win32-ia32*.zip \"MAN_ADS_Client_2.0-Windows_Java_Client.zip\""
        sh "mv MANOVDClient_1.0.0_amd64.deb \"MAN_ADS_Client_2.0-Linux_Java_Client.deb\""
    ...
    dir ('packages') {
        ...
        sh "for i in *.zip; do bash $HOME/github-release.sh github_api_token=${github_token} owner=bacgroup repo=man_ovd_client tag=man_ads_client_${BUILD_NUMBER}_${STAGE} filename=./\$i; done"
        sh "for i in *.deb; do bash $HOME/github-release.sh github_api_token=${github_token} owner=bacgroup repo=man_ovd_client tag=man_ads_client_${BUILD_NUMBER}_${STAGE} filename=./\$i; done"
